      ...
      <main className="max-w-md mx-auto px-6 pt-12 space-y-10">
        ...
        <section className="flex flex-col items-center text-center space-y-4">
          ...
          >
            ...
            <button className="absolute -bottom-2 -right-2 w-10 h-10 bg-slate-950 text-white rounded-2xl flex items-center justify-center shadow-lg border-2 border-white">
              <span className="material-symbols-outlined text-lg">edit</span>
            </button>
          </motion.div>
          
          <div className="space-y-1">
            <h2 className="text-3xl font-black tracking-tighter text-slate-950">Chirag Jeevanani</h2>
            <div className="flex flex-col items-center gap-1.5 pt-1">
              <div className="flex items-center gap-2 bg-emerald-50 px-3 py-1 rounded-full border border-emerald-100">
                ...
                <span className="text-[10px] font-black text-emerald-600 uppercase tracking-widest">+91 98765 43210</span>
              </div>
              <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">chirag@spinzyt.com</p>
            </div>
          </div>
        </section>
        ...
        <section className="space-y-4">
          ...
          <div className="space-y-3">
            {[
              { type: 'Home', addr: 'C-24, Shalimar Township, Indore', icon: 'home' },
              { type: 'Office', addr: 'Crystal IT Park, Tower 2, 4th Floor', icon: 'work' }
            ].map((addr, i) => (
              <motion.div 
                key={i}
                whileTap={{ scale: 0.98 }}
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <p className="text-[13px] font-black text-slate-900 tracking-tight leading-snug">{addr.addr}</p>
                </div>
              </motion.div>
            ))}
          </div>
        ...
        <section className="space-y-4">
          ...
          <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm divide-y divide-slate-50 overflow-hidden">
            {[
              { label: 'UPI ID', val: 'chirag@okhdfc', icon: 'account_balance_wallet' },
              { label: 'Credit Card', val: '**** **** 1234', icon: 'credit_card' },
